...
const ROOT_CA_CN = "OninOneProxy MITM Root CA";
...
async function installCertMac(sudoPassword, certPath) {
  ...
  const deleteOld = `security delete-certificate -c "OninOneProxy MITM Root CA" /Library/Keychains/System.keychain 2>/dev/null || true`;
...
async function installCertLinux(sudoPassword, certPath) {
  ...
  const config = getLinuxCertConfig();
  const destFile = `${config.dir}/oninoneproxy-root-ca.crt`;
