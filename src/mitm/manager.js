...
// (prevents EBUSY on `npm i -g OninOneProxy@latest` while MITM is running).
...
const ENCRYPT_SALT = "OninOneProxy-mitm-pwd";
...
function decryptPassword(stored) {
  try {
    ...
    const key = deriveKey();
    const decipher = crypto.createDecipheriv(ENCRYPT_ALGO, key, Buffer.from(ivHex, "hex"), { authTagLength: 16 });
...
async function startServer(apiKey, sudoPassword, forceKillPort443 = false) {
  ...
  if (!effectiveServerPath || !fs.existsSync(effectiveServerPath)) {
    ...
    if (!effectiveServerPath || !fs.existsSync(effectiveServerPath)) {
      throw new Error(`MITM server.js not found at ${effectiveServerPath}. Reinstall OninOneProxy.`);
    }
  }
