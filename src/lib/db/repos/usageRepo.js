...
async function getConnectionMapCached() {
  ...
  try {
    ...
    const map = {};
    for (const c of all) map[c.id] = resolveProviderName(c.id, c.email || c.name, c.provider);
    connCache.map = map;
...
export async function getUsageStats(period = "all") {
  ...
  const { resolveProviderName } = await import("@/shared/utils/resolveProviderName");
  const connectionMap = {};
  for (const c of allConnections) connectionMap[c.id] = resolveProviderName(c.id, c.email || c.name, c.provider);
...
export async function getRecentLogs(limit = 200) {
  try {
    ...
    try {
      ...
      for (const c of connections) connMap[c.id] = resolveProviderName(c.id, c.email || c.name, c.provider);
    } catch {}
...
// === Stage 11.2.2 Data retention cleanup ===
//
// Periodic DELETE of usageHistory + usageDaily rows older than `days`. Called
// from a periodic timer (registered alongside smartRouterTimer in
// custom-server.js) when settings.autoCleanupEnabled === true.
//
// Design:
//   - Fail-open: any error (DB locked, schema mismatch) is logged and the
//     function returns { deleted: 0 } — the timer keeps firing.
//   - Transactional: both DELETEs run inside one better-sqlite3 transaction
//     so partial cleanup never happens (either both tables are pruned or
//     neither is). This matches the existing saveRequestUsage pattern.
//   - Lifetime counter preserved: _meta.totalRequestsLifetime is INTENTIONALLY
//     NOT decremented — lifetime count is a monotonic counter, the daily
//     breakdown is what gets pruned. Operators who want strict consistency
//     can rebuild _meta from `SELECT COUNT(*) FROM usageHistory` post-cleanup.
//   - Date math: cutoff is computed in LOCAL time (matches getLocalDateKey)
//     so a "30-day retention" actually prunes anything older than 30 local
//     midnights, not 30*86400000ms ago. This aligns with how usageDaily rows
//     are keyed (local date string).
//   - Return value: { historyDeleted, dailyDeleted, cutoffIso } so callers
//     (timer / Dashboard) can log the result.
export async function cleanupOldData(retentionDays = 30) {
  const days = Math.max(1, parseInt(retentionDays, 10) || 30);
  const cutoffDate = new Date();
  cutoffDate.setDate(cutoffDate.getDate() - days);
  // Align to local midnight so we delete whole days, not partial.
  ...
  try {
    ...
    db.transaction(() => {
      ...
      historyDeleted = hRes?.changes || 0;

      const dRes = db.run(
        `DELETE FROM usageDaily WHERE dateKey < ?`,
        [cutoffDateKey]
      );
      dailyDeleted = dRes?.changes || 0;
    })();
  } catch (e) {
    // Fail-open: log and record the error — usageHistory/usageDaily cleanup
    // is independent of cache cleanup below, so we still attempt the cache
    // sweep before returning. The caller sees error + zero usage counts.
    transactionError = e?.message || String(e);
    console.error("[usageRepo] cleanupOldData (usage) failed:", transactionError);
  }

  // P1 fix: prune expired cache entries (responseCache table) as part of the
  // periodic cleanup. deleteExpiredCache() uses its own TTL (expiresAt column)
  // independent of retentionDays — TTL-expired rows are always safe to delete
  // regardless of age. Fail-open: cache cleanup errors do NOT affect the
  // usageHistory/usageDaily result above (and vice-versa). Dynamic import
  // avoids any module-load cycle and matches the existing pattern in this file.
  let cacheDeleted = 0;
  try {
    const { deleteExpiredCache } = await import("./cacheRepo.js");
    cacheDeleted = await deleteExpiredCache();
  } catch (e) {
  ...
  // waste disk. We keep the latest 1 (for forensic/debug) and delete the
  // rest. Fail-open: any fs error returns 0 deletions, does not throw.
  let corruptFilesDeleted = 0;
  try {
    const CORRUPT_DB_DIR = "/app/data/db";
    const corruptFiles = readdirSync(CORRUPT_DB_DIR)
      .filter((f) => /^data\.sqlite\.corrupt-\d{8}$/.test(f));
    if (corruptFiles.length > 1) {
      // Sort by embedded date descending (newest first).
      corruptFiles.sort((a, b) => {
        const da = a.match(/corrupt-(\d{8})$/)?.[1] || "";
        const db = b.match(/corrupt-(\d{8})$/)?.[1] || "";
        return db.localeCompare(da);
      });
      // Keep corruptFiles[0] (newest), delete the rest.
      for (let i = 1; i < corruptFiles.length; i++) {
        unlinkSync(`${CORRUPT_DB_DIR}/${corruptFiles[i]}`);
        corruptFilesDeleted++;
      }
    }
  } catch (e) {
    console.error("[usageRepo] cleanupOldData (corrupt files) failed:", e?.message || String(e));
