...
export async function initializeApp() {
  try {
    ...
    autoStartMitm();
    startQuotaAutoPing();
    // Stage 5.4: hydrate OAuth anti-ban runtime config from persisted settings
    // so the guards pick up the operator's last-known state on container boot.
...
async function autoStartMitm() {
  ...
  try {
    ...
    await startMitm(activeKey?.key || "sk_oninoneproxy", password);
