...
const hasOninOneProxyConfig = (config) => {
  ...
  return config.some((entry) => entry.name === "OninOneProxy");
};

const getOninOneProxyEntry = (config) => {
  if (!Array.isArray(config)) return null;
  return config.find((entry) => entry.name === "OninOneProxy") || null;
};
...
export async function GET() {
  try {
    ...
    const entry = getOninOneProxyEntry(config);

    return NextResponse.json({
      installed: true,
      config,
      hasOninOneProxy: hasOninOneProxyConfig(config),
...
// POST - Apply OninOneProxy config to chatLanguageModels.json
export async function POST(request) {
  try {
    ...
    const keyToUse = apiKey || "sk_oninoneproxy";
    ...
    // Replace existing OninOneProxy entry or append
    const idx = config.findIndex((e) => e.name === "OninOneProxy");
    if (idx >= 0) {
      config[idx] = newEntry;
    } else {
...
// DELETE - Remove OninOneProxy entry from chatLanguageModels.json
export async function DELETE() {
  try {
    ...
    config = config.filter((e) => e.name !== "OninOneProxy");
    await fs.writeFile(configPath, JSON.stringify(config, null, 2));

    return NextResponse.json({
      success: true,
      message: "OninOneProxy removed from Copilot config",
    });
  } catch (error) {
