...
const hasOninOneProxyConfig = (auth) => {
  if (!auth) return false;
  const entry = auth["openai-compatible"] || auth["oninoneproxy"];
  if (!entry) return false;
  const baseUrl = entry.baseUrl || entry.baseURL || "";
  return baseUrl.includes("localhost") || baseUrl.includes("127.0.0.1") || baseUrl.includes("oninoneproxy");
};
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasOninOneProxy: hasOninOneProxyConfig(auth),
...
export async function POST(request) {
  try {
    ...
    try {
      ...
      vscode["kilocode.customProvider"] = { name: "OninOneProxy", baseURL: normalizedBaseUrl, apiKey };
...
export async function DELETE() {
  try {
    ...
    return NextResponse.json({ success: true, message: "OninOneProxy settings removed from Kilo Code" });
  } catch (error) {
