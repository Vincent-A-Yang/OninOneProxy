...
// Check if settings has OninOneProxy customModels
const hasOninOneProxyConfig = (settings) => {
  if (!settings || !settings.customModels) return false;
  return settings.customModels.some(m => m.id?.startsWith("custom:OninOneProxy"));
};
...
// POST - Update OninOneProxy customModels (merge with existing settings)
...
export async function POST(request) {
  try {
    ...
    // Remove all existing OninOneProxy configs
    settings.customModels = settings.customModels.filter(m => !m.id?.startsWith("custom:OninOneProxy"));
...
// DELETE - Remove OninOneProxy customModels only (keep other settings)
export async function DELETE() {
  try {
    ...
    // Remove OninOneProxy customModels
    if (settings.customModels) {
      settings.customModels = settings.customModels.filter(m => !m.id?.startsWith("custom:OninOneProxy"));
