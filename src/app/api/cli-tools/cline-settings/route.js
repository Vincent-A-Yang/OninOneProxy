...
const hasOninOneProxyConfig = (globalState) => {
  if (!globalState) return false;
  const isOpenAi =
    globalState.actModeApiProvider === "openai" || globalState.planModeApiProvider === "openai";
  const baseUrl = globalState.openAiBaseUrl || "";
  return isOpenAi && (baseUrl.includes("localhost") || baseUrl.includes("127.0.0.1") || baseUrl.includes("OninOneProxy"));
};
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasOninOneProxy: hasOninOneProxyConfig(globalState),
...
export async function DELETE() {
  try {
    ...
    return NextResponse.json({ success: true, message: "OninOneProxy settings removed from Cline" });
  } catch (error) {
