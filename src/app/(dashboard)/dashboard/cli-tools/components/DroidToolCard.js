...
export default function DroidToolCard({
  ...
  const getConfigStatus = () => {
    ...
    // Check for any OninOneProxy model entry (support multi-model: custom:OninOneProxy-0, custom:OninOneProxy-1, ...)
    const currentConfig = droidStatus.settings?.customModels?.find(m => m.id?.startsWith("custom:OninOneProxy"));
  ...
  useEffect(() => {
    if (droidStatus?.installed && !hasInitializedModel.current) {
      ...
      const existingModels = (droidStatus.settings?.customModels || [])
        .filter(m => m.id?.startsWith("custom:OninOneProxy"))
        .sort((a, b) => (a.index || 0) - (b.index || 0))
        .map(m => m.model);
      if (existingModels.length > 0) {
        setModelList(existingModels);
      } else {
        // Legacy: single model stored as custom:OninOneProxy-0
        const legacy = droidStatus.settings?.customModels?.find(m => m.id === "custom:OninOneProxy-0");
        if (legacy?.model) {
  ...
  const handleApplySettings = async () => {
    ...
    try {
      const keyToUse = selectedApiKey?.trim()
        ...
        || (!cloudEnabled ? "sk_oninoneproxy" : null);
          ...
          {!checkingDroid && droidStatus?.installed && (
            <>
              <div className="flex flex-col gap-2">
                ...
                {/* Current configured */}
                {droidStatus?.settings?.customModels?.find(m => m.id?.startsWith("custom:OninOneProxy"))?.baseUrl && (
              ...
              <div className="grid grid-cols-1 gap-2 sm:flex sm:items-center">
                ...
                <Button variant="outline" size="sm" onClick={handleResetSettings} disabled={!droidStatus?.hasOninOneProxy} loading={restoring}>
