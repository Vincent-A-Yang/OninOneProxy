...
export default function CodexToolCard({ tool, isExpanded, onToggle, baseUrl, apiKeys, activeProviders, cloudEnabled, initialStatus, tunnelEnabled, tunnelPublicUrl, tailscaleEnabled, tailscaleUrl }) {
  ...
  const handleApplySettings = async () => {
    ...
    setMessage(null);
    try {
      // Use sk_oninoneproxy for localhost if no key, otherwise use selected key
      const keyToUse = (selectedApiKey && selectedApiKey.trim())
        ? selectedApiKey
        : (!cloudEnabled ? "sk_oninoneproxy" : selectedApiKey);
  ...
  const getManualConfigs = () => {
    ...
    const configContent = `# OninOneProxy Configuration for Codex CLI
model = "${selectedModel}"
model_provider = "OninOneProxy"

[model_providers.OninOneProxy]
name = "OninOneProxy"
