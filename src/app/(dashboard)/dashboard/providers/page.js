...
import { isCustomProvider } from "@/lib/providerNormalization";
...
function ApiKeyProviderCard({
  ...
  // True for any user-defined provider (custom-*, openai-compatible-*,
  // anthropic-compatible-*, custom-embedding-*). Used to surface a "Custom"
  // badge in the UI — see OninOneProxy 阶段 4.2.
  const isCustom = isCustomProvider(providerId);
  // Compatible providers already show a Responses/Chat/Messages badge below,
  // so we only render the standalone "Custom" badge for other custom ids
  // (e.g. "custom-foo") to avoid duplicate labels.
  const showCustomBadge = isCustom && !isCompatible && !isAnthropicCompatible;
