...
beforeAll(async () => {
  ...
  // TODO(P2-2): lowdb package not installed in this environment — skip lowdb benchmarks.
  // Re-enable when lowdb is available or remove if no longer needed.
  try {
    ...
    console.log("[db-benchmark] lowdb not available, skipping lowdb benchmarks:", e.message);
  }
});

afterAll(() => {
  // Windows: SQLite file handles may still be open, causing EPERM on rmSync.
  try { if (tempSqlite) fs.rmSync(tempSqlite, { recursive: true, force: true }); } catch { /* EPERM on Windows */ }
  try { if (tempLowdb) fs.rmSync(tempLowdb, { recursive: true, force: true }); } catch { /* EPERM on Windows */ }
  if (originalDataDir === undefined) delete process.env.DATA_DIR;
  else process.env.DATA_DIR = originalDataDir;
});

// P2-2: lowdb package not installed in this environment — all benchmarks require it.
// Re-enable when lowdb is available or remove if no longer needed.
