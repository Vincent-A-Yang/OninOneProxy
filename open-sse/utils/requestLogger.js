...
// Mask sensitive data in headers to avoid leaking tokens/keys in log files.
// Fail-open: any masking error returns the original headers unchanged so the
// request flow never breaks.
function maskSensitiveHeaders(headers) {
  if (!headers) return {};
  try {
    const masked = { ...headers };
    const sensitiveKeys = ["authorization", "x-api-key", "cookie", "token", "apikey"];
