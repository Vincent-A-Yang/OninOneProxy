...
// fail-open: any adapter exception is logged and the original chunk is passed
// through unchanged so the SSE stream never breaks mid-flight.
export function translateResponse(targetFormat, sourceFormat, chunk, state) {
  ...
  if (directFn) {
    try {
      ...
      console.warn(`[translator] direct ${targetFormat}→${sourceFormat} failed; passing chunk through.`, err?.message || err);
      // flush (null chunk) → empty; otherwise passthrough original chunk
  ...
  if (targetFormat !== FORMATS.OPENAI) {
    ...
    if (toOpenAI) {
      try {
        ...
        console.warn(`[translator] ${targetFormat}→openai failed; passing original chunk to source translator.`, err?.message || err);
        // Keep results = [chunk] so step 2 still runs on the original chunk
        // (source-side adapter will see provider-native shape; if it can't
        // handle it, its own fail-open below will pass it through).
      }
    }
  }
    ...
    if (fromOpenAI) {
      ...
      for (const r of results) {
        try {
          ...
          console.warn(`[translator] openai→${sourceFormat} failed; passing intermediate chunk through.`, err?.message || err);
          // Pass through the intermediate (openai-shaped) chunk so the client
          // still receives something rather than a broken stream.
          finalResults.push(r);
        }
      }
...
export function initState(sourceFormat) {
  // Base state for all formats. Fields are pre-initialized so every adapter
  // shares the same state contract — avoids lazy-init pitfalls when two
  // adapters in the same translateResponse pipeline (target→openai→source)
  // touch the same state object.
  const base = {
    messageId: null,
    model: null,
    // Text / thinking block tracking (openai→claude, kiro→claude)
    ...
    nextBlockIndex: 0,
    // Tool call accumulation (openai→claude, kiro→claude buffer args)
    toolCalls: new Map(),
    toolArgBuffers: new Map(),
    // Gemini/Antigravity/Kiro-family silent tool accumulation (plain object keyed by tc.index)
    _toolCallAccum: {},
    // Buffered OpenAI-shape usage (used by gemini/antigravity/kiro adapters)
    _usage: null,
    _responseId: null,
    _modelVersion: null,
    finishReason: null,
...
// D2: Reverse-direction request adapters (source → openai pivot)
import "./request/vertex-to-openai.js";
import "./request/kiro-to-openai.js";
// D2 P1: Alias adapters for CODEX / OPENAI_RESPONSE (reuse Responses API impl)
import "./request/codex-to-openai.js";
import "./request/openai-response-to-openai.js";
...
// D2: Reverse-direction response adapters (openai pivot → source format)
import "./response/openai-to-gemini.js";
import "./response/openai-to-gemini-cli.js";
import "./response/openai-to-vertex.js";
import "./response/openai-to-kiro.js";
import "./response/openai-to-cursor.js";
