...
import { resolveSpoofOverrides } from "../services/oauthAntiBan.js";
...
export class CursorExecutor extends BaseExecutor {
  ...
  buildHeaders(credentials) {
    ...
    // Stage 5.4: apply operator-configured spoof overrides. Cursor's
    // clientVersion rotates with each IDE release; rather than editing the
    // registry on every bump, operators set:
    //   settings.oauthSpoofOverrides.cursor = { clientVersion: "3.2.5" }
    // and this layer maps the override to the x-cursor-client-version header
    // (matching the existing registry.transport.clientVersion semantics).
    // Fail-open: any error here is swallowed so the request still goes out.
    try {
      const overrides = resolveSpoofOverrides("cursor");
      if (overrides && typeof overrides === "object") {
        if (overrides.clientVersion) {
          headers["x-cursor-client-version"] = String(overrides.clientVersion);
        }
