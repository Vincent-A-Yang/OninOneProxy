...
// Stage 11.1.2: bounded LRU for Vertex service-account tokens. Previously an
// unbounded Map — a deployment rotating through many service accounts would
// accumulate entries indefinitely. The ceiling (vertexTokenCacheMaxSize,
// default 100) is far above any realistic deployment (≤ 5 SAs); eviction
// simply forces a re-mint on the next request for an evicted account, which
// is safe (tokens are idempotent to mint within their 1h validity window).
// The LRU returns the stored value directly, so existing `cached.expiresAt`
// / `cached.token` access patterns are preserved.
const vertexTokenCache = createLruMap({
  maxEntries: MEMORY_CONFIG.vertexTokenCacheMaxSize,
});
