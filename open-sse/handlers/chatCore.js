...
export async function handleChatCore({ body, modelInfo, credentials, log, onCredentialsRefreshed, onRequestSuccess, onDisconnect, clientRawRequest, connectionId, userAgent, apiKey, ccFilterNaming, rtkEnabled, headroomEnabled, headroomUrl, headroomCompressUserMessages, headroomAsyncMode, cavemanEnabled, cavemanLevel, ponytailEnabled, ponytailLevel, sourceFormatOverride, providerThinking, isReasoningModel }) {
  ...
  // Models that don't support tools (fail-open: strip to avoid 400)
  if (!passthrough) {
    const caps = getCapabilitiesForModel(provider, model);
    if (caps.tools === false) {
      if (translatedBody.tools) delete translatedBody.tools;
      if (translatedBody.tool_choice) delete translatedBody.tool_choice;
      if (translatedBody.messages) {
        translatedBody.messages = translatedBody.messages.filter(msg => msg.role !== "tool");
      }
      log?.debug?.("TOOLS", `stripped tools for ${provider}/${model} (unsupported)`);
    }
  }
  ...
  // Stage 2.4: when headroomAsyncMode is enabled, fire the compress call in
  // the background without awaiting — the current request is dispatched
  // uncompressed for lowest latency, while Headroom warms its cache so the
  // next identical prompt can hit synchronously. We pass a deep clone so the
  // background task cannot race with the outbound request body.
  const headroomDiagnostics = {};
  let headroomStats = null;
  if (headroomEnabled) {
    if (headroomAsyncMode) {
      let bodySnapshot = null;
      try { bodySnapshot = JSON.parse(JSON.stringify(translatedBody)); } catch { /* skip */ }
      if (bodySnapshot) {
        compressWithHeadroom(bodySnapshot, {
          enabled: true,
          url: headroomUrl,
          model: upstreamModel,
          format: finalFormat,
          ...
          asyncMode: true,
        }).catch(() => { /* fail-open: background refresh swallowed */ });
        headroomDiagnostics.asyncMode = "background";
      } else {
        headroomDiagnostics.reason = "async snapshot failed";
      }
    } else {
    ...
    headroomDiagnostics.reason = "disabled";
  }
